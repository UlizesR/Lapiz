...
LAPIZ_HIDDEN LAPIZ_INLINE int glfw_api_init(LapizState *state) 
{
    ...
    {
        if (state)
            LapizSetError(&state->error, LAPIZ_ERROR_INIT_FAILED, "Failed to initialize GLFW");
...
LAPIZ_API LAPIZ_INLINE void LapizSetWindowflags(unsigned int flags) 
{
    ...
    if (flags & LAPIZ_WINDOW_FOCUSED)
        glfwWindowHint(GLFW_FOCUSED, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_ICONIFIED)
        glfwWindowHint(GLFW_ICONIFIED, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_RESIZABLE)
        glfwWindowHint(GLFW_RESIZABLE, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_VISIBLE)
        glfwWindowHint(GLFW_VISIBLE, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_DECORATED)
        glfwWindowHint(GLFW_DECORATED, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_AUTO_ICONIFY)
        glfwWindowHint(GLFW_AUTO_ICONIFY, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_FLOATING)
        glfwWindowHint(GLFW_FLOATING, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_MAXIMIZED)
        glfwWindowHint(GLFW_MAXIMIZED, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_CENTER_CURSOR)
        glfwWindowHint(GLFW_CENTER_CURSOR, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_TRANSPARENT)
        glfwWindowHint(GLFW_TRANSPARENT_FRAMEBUFFER, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_HOVERED)
        glfwWindowHint(GLFW_HOVERED, GLFW_TRUE);
    if (flags & LAPIZ_WINDOW_FOCUS_ON_SHOW)
