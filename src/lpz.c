...
// Lifecycle documentation lives in Lpz.h alongside the public declarations.
...
struct LpzAppState {
    ...
    // -- Infinite grid pipeline (shader-based, no CPU geometry) ---------------
    lpz_pipeline_t infinite_grid_pipeline;
    lpz_depth_stencil_state_t infinite_grid_ds;
    bool infinite_grid_ready;
    bool infinite_grid_pending;
    float infinite_grid_spacing;
    float infinite_grid_axis_size;
    float infinite_grid_view_proj[16];
    LAPIZ_ALIGN(16) float infinite_grid_inv_view_proj[16];
    bool infinite_grid_vp_dirty;
    ...
    // Persistent maps for the instanced-mesh SSBO (same pattern as point/line).
    void *inst_mapped_ptrs[LPZ_MAX_FRAMES_IN_FLIGHT];
    bool inst_map_valid[LPZ_MAX_FRAMES_IN_FLIGHT];

    // near_fade / far_fade for infinite grid (set from LpzGridDesc in DrawGrid).
    float infinite_grid_near_fade;
    float infinite_grid_far_fade;
};
...
static bool lpz_cpu_push_points(struct LpzAppState *restrict app, const LpzPoint *restrict pts, uint32_t n)
{
    ...
    if (LAPIZ_UNLIKELY(need > app->prim_point_cap_cpu))
    {
...
static bool lpz_cpu_push_lines(struct LpzAppState *restrict app, const LpzLine *restrict lines, uint32_t n)
{
    ...
    if (LAPIZ_UNLIKELY(need > app->prim_line_cap_cpu))
    {
...
{
    uint64_t a, b;
    memcpy(&a, (const char *)m, 8);
    memcpy(&b, (const char *)m + 56, 8);  // bytes 56-63 = floats 14-15
    return a ^ b;
}
...
// Flush one infinite grid draw.  Called from EndDraw before EndRenderPass.
static void lpz_flush_infinite_grid(struct LpzAppState *app)
{
    if (!app->infinite_grid_pending)
        return;
    app->infinite_grid_pending = false;

    if (!app->infinite_grid_pipeline || !app->infinite_grid_ds)
        return;
    ...
    _Static_assert(sizeof(pc) == 80, "infinite grid push-constant must be 80 bytes");

    mat4 vp_mat, inv_vp;
    memcpy(vp_mat, app->infinite_grid_view_proj, 64);
    glm_mat4_inv(vp_mat, inv_vp);
    memcpy(pc.inv_vp, inv_vp, 64);

    pc.spacing = app->infinite_grid_spacing;
    pc.near_fade = app->infinite_grid_near_fade;
    pc.far_fade = app->infinite_grid_far_fade;
    pc._pad = 0.0f;

    app->api.renderer.BeginDebugLabel(app->renderer, "Infinite Grid", 0.5f, 0.8f, 1.0f);
    app->api.renderer.BindPipeline(app->renderer, app->infinite_grid_pipeline);
    app->api.renderer.BindDepthStencilState(app->renderer, app->infinite_grid_ds);
    app->api.renderer.PushConstants(app->renderer, LPZ_SHADER_STAGE_ALL_GRAPHICS, 0, sizeof(pc), &pc);
    app->api.renderer.Draw(app->renderer, 3, 1, 0, 0);
...
static void lpz_build_infinite_grid_pipeline(struct LpzAppState *app)
{
    LpzFormat sc_fmt = app->api.surface.GetFormat(app->surface);
    LpzFormat depth_fmt = app->enable_depth ? LPZ_FORMAT_DEPTH32_FLOAT : LPZ_FORMAT_UNDEFINED;

    lpz_shader_t gvs = NULL, gfs = NULL;
    if (app->use_metal)
    {
        LpzFileBlob blob = lpz_find_default_blob(true, "grid.metal");
        if (!blob.data)
        {
            LPZ_LOG_WARNING(LPZ_LOG_CATEGORY_SHADER, "LpzApp: grid.metal not found — DrawInfiniteGrid disabled.");
            return;
        }
        gvs = lpz_compile_blob(app, blob, "grid_vertex", LPZ_SHADER_STAGE_VERTEX);
        gfs = lpz_compile_blob(app, blob, "grid_fragment", LPZ_SHADER_STAGE_FRAGMENT);
        LpzIO_FreeBlob(&blob);
    }
    else
    {
        LpzFileBlob vb = lpz_find_default_blob(false, "spv/grid.vert.spv");
        LpzFileBlob fb = lpz_find_default_blob(false, "spv/grid.frag.spv");
        if (!vb.data || !fb.data)
        {
            LPZ_LOG_WARNING(LPZ_LOG_CATEGORY_SHADER, "LpzApp: grid SPIR-V not found — DrawInfiniteGrid disabled.");
            LpzIO_FreeBlob(&vb);
            LpzIO_FreeBlob(&fb);
            return;
        }
        gvs = lpz_compile_blob(app, vb, "main", LPZ_SHADER_STAGE_VERTEX);
        gfs = lpz_compile_blob(app, fb, "main", LPZ_SHADER_STAGE_FRAGMENT);
        LpzIO_FreeBlob(&vb);
        LpzIO_FreeBlob(&fb);
    }
    ...
    if (!gvs || !gfs)
    {
        LPZ_LOG_ERROR(LPZ_LOG_CATEGORY_SHADER, LPZ_SHADER_COMPILE_FAILED, "LpzApp: grid shader compilation failed.");
        if (gvs)
            app->api.device.DestroyShader(gvs);
        if (gfs)
            app->api.device.DestroyShader(gfs);
        return;
    }

    // Depth test OFF — grid renders as a background/overlay layer.
    ...
    // No vertex attributes, no bind groups — everything via push constants.
    app->api.device.CreatePipeline(app->device,
                                   ...
                                   &app->infinite_grid_pipeline);

    app->api.device.DestroyShader(gvs);
    app->api.device.DestroyShader(gfs);

    if (app->infinite_grid_pipeline)
    {
        app->infinite_grid_ready = true;
        LPZ_LOG_INFO(LPZ_LOG_CATEGORY_GENERAL, "LpzApp: infinite grid pipeline ready.");
    }
    else
    {
...
LpzResult InitApp(const LpzAppDesc *desc, int argc, char **argv, lpz_app_t *out)
{
    ...
    app->enable_depth = (desc->width == 0) || desc->enable_depth;
...
{
    if (!app || !paths || !out_vert || !out_frag)
        return LPZ_INVALID_ARGUMENT;
...
LpzResult CreateContext(lpz_app_t app)
{
    ...
    app->infinite_grid_vp_dirty = true;  // force inv_vp compute on first DrawGrid
    app->infinite_grid_near_fade = 20.0f;
    app->infinite_grid_far_fade = 80.0f;
    app->run = true;
...
void DestroyContext(lpz_app_t app)
{
    ...
    LPZ_FREE(app->prim_point_cpu);
    LPZ_FREE(app->prim_line_cpu);
    LPZ_FREE(app->grid_cache);
    ...
    if (app->infinite_grid_pipeline)
        app->api.device.DestroyPipeline(app->infinite_grid_pipeline);
    if (app->infinite_grid_ds)
        app->api.device.DestroyDepthStencilState(app->infinite_grid_ds);
    app->infinite_grid_ready = false;

    // Instance buffer
    if (app->inst_buf)
    {
        for (uint32_t _s = 0; _s < LPZ_MAX_FRAMES_IN_FLIGHT; _s++)
            if (app->inst_map_valid[_s])
                app->api.device.UnmapMemory(app->device, app->inst_buf, _s);
    }
    if (app->inst_bg)
...
LpzResult BeginDraw(lpz_app_t app, LpzFrameInfo *out_frame)
{
    ...
    // Both guards use LAPIZ_UNLIKELY so the steady-state (no trim) path is
    // predicted not-taken by the branch predictor.
    {
        const uint32_t MIN_CAP = 256u;
        const uint32_t SHRINK_FACTOR = 4u;
        if (LAPIZ_UNLIKELY(app->prim_point_cap_cpu > MIN_CAP))
        {
            ...
            {
                uint32_t new_cap = LPZ_MAX(MIN_CAP, app->prim_point_peak * 2u);
        ...
        if (LAPIZ_UNLIKELY(app->prim_line_cap_cpu > MIN_CAP))
        {
            ...
            {
                uint32_t new_cap = LPZ_MAX(MIN_CAP, app->prim_line_peak * 2u);
...
void DrawMeshInstanced(lpz_app_t app, const Mesh *mesh, const void *instance_data, uint32_t instance_stride, uint32_t instance_count)
{
    ...
    if (stale)
    {
        ...
        // Buffer recreated — invalidate all persistent slot maps.
        memset(app->inst_map_valid, 0, sizeof(app->inst_map_valid));
    }

    uint32_t inst_slot = app->current_frame_index;
    if (LAPIZ_UNLIKELY(!app->inst_map_valid[inst_slot]))
    {
        app->inst_mapped_ptrs[inst_slot] = app->api.device.MapMemory(app->device, app->inst_buf, inst_slot);
...
void DrawPointCloud(lpz_app_t app, mat4 mvp, const LpzPoint *points, uint32_t count)
{
    ...
    uint64_t _mvp_h = lpz_mvp_hash(mvp);
    if (_mvp_h != app->prim_mvp_hash)
    {
        memcpy(app->prim_mvp, mvp, 64);
        app->prim_mvp_hash = _mvp_h;
    }
...
void DrawPoint(lpz_app_t app, mat4 mvp, vec3 position, vec4 color, float size)
{
    if (!app || !mvp || !app->prim_gpu_ready)
        return;
    {
        uint64_t _h = lpz_mvp_hash(mvp);
        if (_h != app->prim_mvp_hash)
        {
            memcpy(app->prim_mvp, mvp, 64);
            app->prim_mvp_hash = _h;
        }
    }
...
{
    if (!app || !view_proj || !desc || !app->prim_gpu_ready)
        return;
    if (desc->flags == 0)
        return;

    // ── Bounded grid + axes (CPU line geometry) ───────────────────────────────
    if ((desc->flags & LPZ_GRID_DRAW_GRID) || (desc->flags & LPZ_GRID_DRAW_AXES))
    {
        {
            uint64_t _h = lpz_mvp_hash(view_proj);
            if (_h != app->prim_mvp_hash)
            {
                memcpy(app->prim_mvp, view_proj, 64);
                app->prim_mvp_hash = _h;
            }
        }

        bool rebuild = !app->grid_cache_valid || app->grid_cache_grid_size != desc->grid_size || app->grid_cache_axis_size != desc->axis_size || app->grid_cache_thickness != desc->thickness || app->grid_cache_flags != (uint32_t)desc->flags;

        if (rebuild)
        {
            int gs = desc->grid_size;
            float fg = (float)gs;
            float fa = desc->axis_size;
            float tk = desc->thickness;
            // Grid lines are half the axis thickness; centre lines 75%.
            float grid_tk = tk * 0.5f;
            float centre_tk = tk * 0.75f;

            // Dim grey for ordinary grid lines; brighter for centre lines.
            const float GR = 0.28f, GG = 0.28f, GB = 0.28f, GA = 0.30f;
            const float CR = 0.52f, CG = 0.52f, CB = 0.52f, CA = 0.55f;

            // Count lines: 2*(2*gs+1) for the grid + 3 for axes.
            uint32_t grid_lines = ((desc->flags & LPZ_GRID_DRAW_GRID) && gs > 0) ? (uint32_t)(4 * gs + 2) : 0u;
            uint32_t axis_lines = (desc->flags & LPZ_GRID_DRAW_AXES) ? 3u : 0u;
            ...
            if ((desc->flags & LPZ_GRID_DRAW_GRID) && gs > 0)
            {
                // Z-axis lines (parallel to X): one per z row.
                for (int z = -gs; z <= gs; z++)
                {
                    float fz = (float)z;
                    if (z == 0)
                        // Centre line: brighter and slightly thicker.
                        L[i++] = (LpzLine){.start = {-fg, 0, 0}, .end = {fg, 0, 0}, .color = {CR, CG, CB, CA}, .thickness = centre_tk};
                    else
                        L[i++] = (LpzLine){.start = {-fg, 0, fz}, .end = {fg, 0, fz}, .color = {GR, GG, GB, GA}, .thickness = grid_tk};
                }
                // X-axis lines (parallel to Z): one per x column.
                for (int x = -gs; x <= gs; x++)
                {
                    float fx = (float)x;
                    if (x == 0)
                        L[i++] = (LpzLine){.start = {0, 0, -fg}, .end = {0, 0, fg}, .color = {CR, CG, CB, CA}, .thickness = centre_tk};
                    else
                        L[i++] = (LpzLine){.start = {fx, 0, -fg}, .end = {fx, 0, fg}, .color = {GR, GG, GB, GA}, .thickness = grid_tk};
                }
            }

            if (desc->flags & LPZ_GRID_DRAW_AXES)
            {
                // Short positive-direction arrows from origin — axis_size independent.
                L[i++] = (LpzLine){.start = {0, 0, 0}, .end = {fa, 0, 0}, .color = {1.0f, 0.25f, 0.25f, 1.0f}, .thickness = tk};
                L[i++] = (LpzLine){.start = {0, 0, 0}, .end = {0, fa, 0}, .color = {0.25f, 1.0f, 0.25f, 1.0f}, .thickness = tk};
                L[i++] = (LpzLine){.start = {0, 0, 0}, .end = {0, 0, fa}, .color = {0.25f, 0.5f, 1.0f, 1.0f}, .thickness = tk};
            }

            app->grid_cache_grid_size = desc->grid_size;
            app->grid_cache_axis_size = desc->axis_size;
            app->grid_cache_thickness = desc->thickness;
            app->grid_cache_flags = (uint32_t)desc->flags;
...
infinite_pass:
    // ── Infinite grid (deferred shader draw) ─────────────────────────────────
    if (desc->flags & LPZ_GRID_INFINITE)
    {
        if (!app->infinite_grid_ready)
        {
            LPZ_LOG_WARNING(LPZ_LOG_CATEGORY_RENDERER, "DrawGrid: LPZ_GRID_INFINITE requested but grid pipeline not ready (grid.metal / grid.vert.spv + grid.frag.spv not found).");
            return;
        }
        memcpy(app->infinite_grid_view_proj, view_proj, 64);
        app->infinite_grid_spacing = desc->spacing > 0.0f ? desc->spacing : 1.0f;
        app->infinite_grid_axis_size = desc->axis_size;
        app->infinite_grid_pending = true;
    }
}
