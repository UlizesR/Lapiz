...
static int MetalHashLookup(const MetalUniformHash* h, const char* name)
{
    ...
    for (int probe = 0; probe < LAPIZ_UNIFORM_HASH_SIZE; probe++)
    {
        ...
        if (ei == LAPIZ_UNIFORM_HASH_EMPTY) return -1;
...
LapizShader* LapizMTLShaderLoadFromMemory(const char* vsCode, const char* fsCode)
{
    ...
    if (!mtl_s || !mtl_s->device) return NULL;
    ...
    if (!lib)
    {
        ...
        if (msg) { strncpy(g_metal_shader_error, msg, LAPIZ_SHADER_ERROR_MAX - 1); g_metal_shader_error[LAPIZ_SHADER_ERROR_MAX - 1] = '\0'; }
    ...
    id<MTLFunction> vs = [lib newFunctionWithName:@(kMetalVertexEntry)];
    id<MTLFunction> fs = [lib newFunctionWithName:@(kMetalFragmentEntry)];
...
LapizShader* LapizMTLShaderLoadFromFile(const char* vertPath, const char* fragPath)
{
    ...
    LapizShader* shader = LapizMTLShaderLoadFromMemory(source, NULL);
    free(source);
    return shader;
}
