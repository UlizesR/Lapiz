...
static int GlGetCachedLocation(GlUniformCache *cache, unsigned int program, const char *name) 
{
    ...
    if (cache->count >= LAPIZ_UNIFORM_CACHE_MAX)
        return loc;
...
LapizShader *LapizGLShaderLoadDefault(void) 
{
    ...
    unsigned int vs = compile_glsl(default_vert_glsl, GL_VERTEX_SHADER);

    if (!vs)
        return NULL;

    unsigned int fs = compile_glsl(default_frag_glsl, GL_FRAGMENT_SHADER);
...
{
    g_gl_shader_error[0] = '\0';
    const char *vs = vsCode ? vsCode : default_vert_glsl;
    const char *fs = fsCode ? fsCode : default_frag_glsl;
...
LapizShader *LapizGLShaderLoadFromFileEx(const char *vertPath, const char *fragPath, const char *vertEntry, const char *fragEntry) 
{
    (void)vertEntry;
    (void)fragEntry;
    return LapizGLShaderLoadFromFile(vertPath, fragPath);
}
...
{
    if (!shader || !shader->program || !uniformName)
        return -1;
...
{
    if (!shader || !shader->program || loc < 0)
        return;
...
{
    if (!shader || !shader->program || loc < 0 || !v)
        return;
...
{
    if (!shader || !shader->program || loc < 0 || !m)
        return;
...
{
    if (!shader || !shader->program || loc < 0 || !texture || !texture->_backend)
        return;
...
{
    if (!shader || idx < 0 || idx >= LAPIZ_SHADER_LOC_COUNT)
        return -1;
