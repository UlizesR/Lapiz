...
LAPIZ_HIDDEN LapizResult LapizGLInit(LapizWindow *window)
{
    ...
    gl_s->default_shader = NULL;  /* Created lazily when LapizGLDrawFullscreen is first called */
...
LAPIZ_HIDDEN void LapizGLBeginDraw(void)
{
    ...
    if (gl_s->use_depth) clear_bits |= GL_DEPTH_BUFFER_BIT;
    glClear(clear_bits);
}
...
{
    if (!gl_s || !gl_s->fullscreen_vao) return;
    if (!gl_s->default_shader)
    {
        gl_s->default_shader = LapizGLShaderLoadDefault();
        if (!gl_s->default_shader) return;
    }
    LapizGLShaderUse(gl_s->default_shader);
    int colorLoc = LapizGLShaderGetDefaultLocation(gl_s->default_shader, LAPIZ_SHADER_LOC_COLOR);
    if (colorLoc >= 0)
        LapizGLShaderSetColor(gl_s->default_shader, colorLoc, gl_s->clear_color);
